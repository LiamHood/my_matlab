...
[t_new1 ,state_obj1] = ode45(@OrbitStep, t_span1, state1, options, mu);
...
options = odeset('RelTol', 1e-8,'AbsTol', 1e-8);
[t_new2 ,state_obj1] = ode45(@OrbitStep, t_span1, statemove, options, mu);

% *********** FROM THIS MOMENT ON MISSION IN PROGRESS **************

element = length(state_obj1);
...
r_geo_start = [state_obj1(end,1), state_obj1(end,2), state_obj1(end,3)]; %these vecs grab all values. element was stopping short
v_geo_start = [state_obj1(end,4), state_obj1(end,5), state_obj1(end,6)];
...
t_span1 = [0 t_add];
[t_new ,state_obj1] = ode45(@OrbitStep, t_span1, state1, options, mu);
%Geo plot
figure
view(43,24);
[x,y,z] = sphere;
...
% end
hold on 
% plot3(state_obj1(:,1), state_obj1(:,2), state_obj1(:,3) , '-k' );
% plot3(state_obj1(1,1), state_obj1(1,2), state_obj1(1,3), '*') %plot start
...
[r_pmeo,v_pmeo,r_gmeo,v_gmeo] = COEs2State(COE2(4), COE2(2), COE1(3), COE1(5), h2, TA2, mu);
% [epoch,inc,RAAN,ecc,ArgPer,MA,n];

%Plot of MEO orbit
t_span1 = [0 T2];
state_new2 = [r_gmeo, v_gmeo];
[t_new1 ,state_obj2] = ode45(@OrbitStep, t_span1, state_new2, options, mu);
% plot3(state_obj2(:,1), state_obj2(:,2), state_obj2(:,3), '-k')

[h2,inc2,N2,RAAN2,ecc2,ArgPer2,TA2,SMA2,T2,n2] = Orbital_elements(r_gmeo,v_gmeo,mu);
...
deltav1 = sqrt(va_orbit1^2 + va_tran^2 - 2*va_orbit1*va_tran*cos(deltainc)); % changing inc and getting onto transfer orbit
                                ...
                                incTrans1 = COE2(2) ;       
                                [~,~,r_Trans1,v_Trans1] = COEs2State(ecctran, incTrans1, deg2rad(RAAN1), deg2rad(ArgPer1) , htran, pi, mu) ;
       [htran,inctran,Ntran,RAANtran,ecctran,ArgPertran,TAtran,SMAtran,Ttran,ntran] = Orbital_elements(r_Trans1,v_Trans1,mu); 
       

halftrantime = Ttran*0.5;

%plot of transfer orbit with inc change
state_trans = [r_Trans1',v_Trans1'];
t_span = [0 Ttran];
[t_new2 ,state_trans1] = ode45(@OrbitStep, t_span, state_trans, options, mu);
hold on
% plot3(state_trans1(:,1), state_trans1(:,2), state_trans1(:,3) ,'-b')

Time_since_mission_start = Time_since_mission_start + halftrantime;
...
[r_p2,v_p2,r_geo2,v_geo2] = COEs2State(COE2(4), COE2(2), COE1(3), COE1(5), h2, TA2, mu); % getting state vector of MEO object from TLE
...
[t_new2 ,state_new2] = ode45(@OrbitStep, t_span_move2, statemove2, options, mu); % getting new state vector of MEO object at this time in mission 
 %Meo plot
%  Is plotted in GEO calcs along with transfer orbit #1
...
Emeo = 2*atan(sqrt((1-eccmeo)/(1+eccmeo))*tand(TAmeo/2)); % eccentric anomaly for MEO orbit
...
eccphase_meo = (orbit2_periapsis - ra_phasemeo)/(orbit2_periapsis + ra_phasemeo) ; % eccentricity of the phase orbit
hphase_meo = sqrt(ra_phasemeo*mu*(1+eccphase_meo)) ; % specific angular momentum of the phase orbit

                % COES and state of phasing
                [~,~,r_Phase1,v_Phase1] = COEs2State(eccphase_meo, deg2rad(incmeo), deg2rad(RAANmeo), deg2rad(ArgPermeo), hphase_meo, 0, mu) ;
                [hphase,incphase,Nphase,RAANphase,eccphase,ArgPerphase,TAphase,SMAphase,Tphase,nphase] = Orbital_elements(r_Phase1,v_Phase1,mu);   
                
                statephase = [r_Phase1',v_Phase1'];
                t_spanphase = [0 Tphase];
                [t_newphase, state_phase1] = ode45(@OrbitStep, t_spanphase, statephase, options, mu);
                hold on
%                 plot3(state_phase1(:,1), state_phase1(:,2), state_phase1(:,3) ,'-r')
...
%Meo plot already plotted above
% hold on
...
[r_p3,v_p3,r_geo3,v_geo3] = COEs2State(COE3(4), COE3(2), COE1(3), COE1(5), h3, TA3, mu); % getting state vector of LEO object from TLE

statemove3(1:3) = r_geo3';
statemove3(4:6) = v_geo3';
options = odeset('RelTol', 1e-8,'AbsTol', 1e-8);
[t_new6 ,state_obj3] = ode45(@OrbitStep, t_span_move3, statemove3, options, mu); % getting new state vector of LEO object at this time in mission 


element3 = length(state_obj3);
r_leo1 = [state_obj3(element3,1), state_obj3(element3,2), state_obj3(element3,3)]; % position vector of leo object
v_leo1 = [state_obj3(element3,4), state_obj3(element3,5), state_obj3(element3,6)]; % velocity vector of leo object
...
rp_trans1 = (ra_meo + ra_leo1)/2 ; % rp of first transfer ellipse

e_trans1 = (orbit2_periapsis - rp_trans1)/(orbit2_periapsis + rp_trans1) ; % eccentricity of the first transfer ellipse
a_trans1 = (orbit2_periapsis + rp_trans1)/2 ; % semi major axis of first transfer ellipse
T_trans1 = ((2*pi)/(sqrt(mu)))*(a_trans1^(3/2)) ; % period of first transfer ellipse
h_trans1 = sqrt(rp_trans1*mu*(1+e_trans1)) ; % specific angular momentum of first transfer ellipse


                % COEs and state of first ellipse
                TA_trans1 = 0 ;
                [~,~,r_ellipse1,v_ellipse1] = COEs2State(e_trans1, deg2rad(inc2), deg2rad(RAAN1), deg2rad(ArgPer1), h_trans1, TA_trans1, mu) ;
                        [hel1,incel1,Nel1,RAANel1,eccel1,ArgPerel1,TAel1,SMAel1,Tel1,nel1] = Orbital_elements(r_ellipse1,v_ellipse1,mu) ;            
                         state_el3 = [r_ellipse1, v_ellipse1];
                        T_spanel1 = [0 0.5*Tel1];
                         [t_newelip1 ,state_elip1] = ode45(@OrbitStep, T_spanel1, state_el3, options, mu);
                         hold on
%                          plot3(state_elip1(:,1), state_elip1(:,2), state_elip1(:,3) , '-b' )
                        
                        
e_trans2 = (rp_trans1 - ra_leo1)/(rp_trans1 + ra_leo1) ; % eccentricity of the 2nd transfer ellipse
a_trans2 = (ra_leo1 + rp_trans1)/2 ; % semi major axis of 2nd transfer ellipse
T_trans2 = ((2*pi)/(sqrt(mu)))*(a_trans2^(3/2)) ; % period of 2nd transfer ellipse
h_trans2 = sqrt(rp_trans1*mu*(1-e_trans2)) ; % specific angular momentum of 2nd transfer ellipse

                % COEs and state of first ellipse
                TA_trans2 = 0 ;
                [~,~,r_ellipse2,v_ellipse2] = COEs2State(e_trans2, deg2rad(inc2), deg2rad(RAAN1), deg2rad(ArgPer1)+(pi), h_trans2, pi, mu) ;
                [hel2,incel2,Nel2v,RAANel2,eccel2,ArgPerel2,TAel2,SMAel2,Tel2,nel2] = Orbital_elements(r_ellipse2,v_ellipse2,mu) ;
                                    state_el3 = [r_ellipse2, v_ellipse2];
                        tt = 0.5*Tel2;
                                    T_spanel1 = [0 Tel2];
                         [t_newelip2 ,state_elip2] = ode45(@OrbitStep, T_spanel1, state_el3, options, mu);
                         one = flip(state_elip2(213:429,1));
                         two = flip(state_elip2(213:429,2));
                         three = flip(state_elip2(213:429,3));
                         hold on
%                          plot3(one, two, three , '-b' )
                         
v1_trans1 = h_trans1/orbit2_periapsis ; % first v burn onto transfer 1
v1_trans2 = h_trans2/rp_trans1 ; % second v burn onto transfer 2
v2_trans2 = h3/rp_leo1 ; % third v burn onto transfer 3
...
% first LEO Object
        ...
        SMA3 = (mu/(n3)^2)^(1/3); % Semi-major Axis (km)
        E3 = eccentric_anomaly(ecc3, MA3); % Calculating Eccentric Anomaly 
        TA3 = 2*pi + 2*atan( tan(E/2) / sqrt((1-ecc3)/(1+ecc3)) ); % Calculating TA in radians
        h3 = sqrt(SMA3*mu*(1-ecc3^2)); % Angular Momentum
    ...
    % Phase to first LEO sat
        ...
        vp3phase = mu/h3phase*(1+ecc3phase) ;
                            ...
                            [ t_phase2 , state_phase2] = ode45(@OrbitStep, [ 0 , T_phase3 ] , [ r_phase2 ; v_phase2 ], options, mu);                 
%                             plot3( state_phase2(:,1) , state_phase2(:,2) , state_phase2(:,3) , '-r')                                
...
state_3o = [r_3,v_3];
t_span = [0 T_phase3];
[t_new8 ,state_3] = ode45(@OrbitStep, t_span, state_3o, options, mu);
hold on
% plot3(state_3(:,1), state_3(:,2), state_3(:,3) , '-k' )
    % second LEO
...
[~,~,r_4,v_4] = COEs2State(COE4(4), COE4(2), COE4(3), COE4(5) , h4 , 0 , mu); % find state of 4th object after transfer time
state_4o = [r_4,v_4];
t_span = [0 T4];
[t_new8 ,state_4] = ode45(@OrbitStep, t_span, state_4o, options, mu);
% plot3(state_4(:,1), state_4(:,2), state_4(:,3) , '-k' )
 % ********************************************************
 
%  Graph Legend

% Blue = GEO
% Neon Green = 1st Transfer
% Red = MEO
% Magenta = Phasing orbit
% Purple = 1st elliptical
% Dark Green = 2nd elliptical
% Yellow = 1st LEO
% Light Blue = 2nd LEO

% DARK RED = I DONT KNOW 
...
%  *********************************************************
    ...
    %transfer from first to second LEO object     
         t_L2Lall = linspace( 5e2 , 3e4 , 5e3 ); % time to transfer from LEO to LEO
       ...
       % Use Lamberts to transfer
            ...
            [ v1(:,ii) , v2(:,ii)  ] = Lamberts2( r_3 , r_4 , t_L2L , mu , 1e-5 , 1 ) ;
            tspan = [ 0 t_L2Lall(ii) ] ;
            [ tT , stateT ] = ode45( @OrbitStep , tspan , [ r_3 , v1(:,ii) ] , options , mu ) ;
            dvBegr = norm( v1(:,ii) - v_3 ) ;
            dvEndr = norm( v2(:,ii) - v_4 ) ;
            dv(ii) = dvBegr+dvEndr ;
                r = norm( stateT( length(stateT) , 1:3 ) );
                if r < 6578 
                    dv(ii) = 100 ;
                end                
        end
        ...
        Time_since_mission_start = Time_since_mission_start + t_L2Lall(i) + T4*5 ;
        MissionTime = Time_since_mission_start/(60*60*24) ;
          % plot trans orbit
state_trans = [r_3,v1(:,i)];
t_span = [0 t_L2Lall(i)];
[t_new9 ,state_L2L] = ode45(@OrbitStep, t_span, state_trans, options, mu);
hold on
% plot3(state_L2L(:,1), state_L2L(:,2), state_L2L(:,3) , '-b' ) 
...
zlabel('Z (km)');
hold on
plot3( state_obj1(:,1) , state_obj1(:,2) , state_obj1(:,3) )
plot3( state_obj2(:,1) , state_obj2(:,2) , state_obj2(:,3) )
plot3( state_trans1(:,1) , state_trans1(:,2) , state_trans1(:,3) )
legend( 'Earth' , 'Obj1' , 'Obj2' , 'Transfer1' )
hold off
...
zlabel('Z (km)');
hold on
plot3( state_phase1(:,1) , state_phase1(:,2) , state_phase1(:,3) , '-g' )
plot3( state_obj2(:,1) , state_obj2(:,2) , state_obj2(:,3) , '-b' )
plot3( state_3(:,1) , state_3(:,2) , state_3(:,3) , '-b' )
plot3( state_elip1(:,1) , state_elip1(:,2) , state_elip1(:,3) , '-r')
plot3( one , two , three , '-r' )
legend( 'Earth' , 'Phasing' , 'MEO Debris (GIOVE-A)' , 'Cosmos 2251 Debris' , 'Ellipse 1' , 'Ellipse 2' )
hold off
...
zlabel('Z (km)');
hold on
plot3( state_phase2(:,1) , state_phase2(:,2) , state_phase2(:,3) , '-g' )
plot3( state_3(:,1) , state_3(:,2) , state_3(:,3) , '-b' )
plot3( state_L2L(:,1) , state_L2L(:,2) , state_L2L(:,3) , '-r')
plot3( state_4(:,1) , state_4(:,2) , state_4(:,3) , '-k' )
legend( 'Earth' , 'Phasing' , 'Cosmos 2251 Debris' , 'Transfer' , 'Jason-1' )
hold off
...
zlabel('Z (km)');
hold on
plot3(state_obj1(:,1), state_obj1(:,2), state_obj1(:,3) , '-b' );
plot3(state_obj1(1,1), state_obj1(1,2), state_obj1(1,3), '*') %plot start
plot3(state_obj2(:,1), state_obj2(:,2), state_obj2(:,3), '-b')
plot3(state_trans1(:,1), state_trans1(:,2), state_trans1(:,3) ,'-r')
plot3(state_phase1(:,1), state_phase1(:,2), state_phase1(:,3) ,'-b')
plot3(state_elip1(:,1), state_elip1(:,2), state_elip1(:,3) , '-r' )
plot3(one, two, three , '-r' )
plot3( state_phase2(:,1) , state_phase2(:,2) , state_phase2(:,3) , '-b')               
plot3(state_3(:,1), state_3(:,2), state_3(:,3) , '-b' )
plot3(state_4(:,1), state_4(:,2), state_4(:,3) , '-k' )
plot3(state_L2L(:,1), state_L2L(:,2), state_L2L(:,3) , '-r' )
legend( 'Earth' , 'GEO Debris (ASIASAT 3S)' , 'Start Position' , 'MEO Debris (GIOVE-A)' , 'Transfer from ASIASAT 3S to GIOVE-A' , 'Phase to GIOVE-A' , 'Ellipse 1 to Cosmos 2251 Debris' , 'Ellipse 2 to Cosmos 2251 Debris' , 'Phase to Cosmos 2251 Debris' , 'LEO Debris #1 (Cosmos 2251 Debris)' , 'LEO Debris #2 (Jason-1)' , 'Transfer from Cosmos 2251 to Jason-1' )
hold off
