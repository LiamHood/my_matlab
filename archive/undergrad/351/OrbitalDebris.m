...
% epoch = COE1(1);
% Tnaught = (epoch - 2451545)/36525;

% COEs (check)

% [h,inc,N,RAAN,ecc,ArgPer,TA,SMA,T,n] = Orbital_elements(r_geo1,v_geo1,mu);
...
% figure(1)
% hold on
% plot3(r_geo1(1,1),r_geo1(2,1),r_geo1(3,1),'*')
% plot3(state_new1(:,1),state_new1(:,2),state_new1(:,3))
% title('3D View Of Orbit')
% grid on
% xlabel('x,km')
% ylabel('y,km')
% zlabel('z,km')
...
t_span1 = [0 T1];
options = odeset('RelTol', 1e-8,'AbsTol', 1e-8);
[t_new1 ,state_new1] = ode45(@OrbitStep, t_span_move, statemove, options, mu);
...
t_span_meo = [0 t_meo_per];

statemeo2(1:3) = r_meo';
statemeo2(4:6) = v_meo';
options = odeset('RelTol', 1e-8,'AbsTol', 1e-8);
[t_meo2 ,state_meo2] = ode45(@OrbitStep, t_span_meo, statemeo2, options, mu); % getting new state vector of MEO object at this time in mission 
...
options = odeset('RelTol', 1e-8,'AbsTol', 1e-8);
[t_new3 ,state_new3] = ode45(@OrbitStep, t_span_move3, statemove3, options, mu); % getting new state vector of LEO object at this time in mission 
...
e_trans2 = (rp_trans1 - rp_leo1)/(rp_trans1 + rp_leo1) ; % eccentricity of the 2nd transfer ellipse
a_trans2 = (rp_leo1 + rp_trans1)/2 ; % semi major axis of 2nd transfer ellipse
...
%         deltav_tot = dv3phase + deltav_tot ; % update total
...
%             [ tT , stateT ] = ode45( @TwoBodyMotion , tspan , [ r_3 , v1(:,ii) ] , options , mu ) ;
%             dvBeg = norm( v1(:,ii) - v_3 ) ;
%             dvEnd = norm( v2(:,ii) - v_4 ) ;
%             dv(ii) = dvBeg+dvEnd ;
%                 r = norm( stateT( length(stateT) , 1:3 ) );
%                 if r < 6578 
%                     dv(ii) = 100 ;
%                 end
%             [ v1r(:,ii) , v2r(:,ii)  ] = Lamberts2( r_3 , r_4 , t_L2L , mu , 1e-5 , 0 ) ;
%             tspan = [ 0 t_L2Lall(ii) ] ;
%             options = [ 'AbsTol' , 1e-10 , 'RelTol' , 1e-10 ] ;
%             [ tT , stateT ] = ode45( @TwoBodyMotion , tspan , [ r_3 , v1r(:,ii) ] , options , mu ) ;
%             dvBegr = norm( v1r(:,ii) - v_3 ) ;
%             dvEndr = norm( v2r(:,ii) - v_4 ) ;
%             dvr(ii) = dvBegr+dvEndr ;
%                 r = norm( stateT( length(stateT) , 1:3 ) );
%                 if r < 6578 
%                     dv(ii) = 100 ;
%                 end                
...
%         % Optimization
%         figure
%             plot( t_L2Lall , dv , t_L2Lall , dvr )
%             ylabel( 'Delta v (km/s)' )
%             xlabel( 'Transfer Time (s)' )
%             legend( 'Prograde' , 'Retrograde' )
%             [ dvOpt , i ] = min( dv ) ;
...
%         tspan = [ 0 t_L2Lall(jj) ] ;
%         options = [ 'AbsTol' , 1e-10 , 'RelTol' , 1e-10 ] ;
%         [ tT , stateT ] = ode45( @TwoBodyMotion , tspan , [ r_3 , v1(:,jj) ] , options , mu ) ;
%         hold on
%         plot3( stateT(:,1) , stateT(:,2) , stateT(:,3) , '-b' )
%     end
%         
%     for jj = 1:ii
%         tspan = [ 0 t_L2Lall(jj) ] ;
%         options = [ 'AbsTol' , 1e-10 , 'RelTol' , 1e-10 ] ;
%         [ tTr , stateTr ] = ode45( @TwoBodyMotion , tspan , [ r_3 , v1r(:,jj) ] , options , mu ) ;
%         hold on
%         plot3( stateTr(:,1) , stateTr(:,2) , stateTr(:,3) , '-r' )
%     end
%         xlabel( 'X (km)' )
%         ylabel( 'Y (km)' )
%         zlabel( 'Z (km)' )
