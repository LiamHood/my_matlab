...
r = [ -3024.75026 ; -5427.4680  ; 2738.15240 ] ;
v = [ 3.385735654 ; -4.509643617 ; -5.191137996 ] ;
% This is the State Vector in ECI

%Omega Vector Initial
Omega = [ 0.1651 ; 0.1234 ;  0.0004 ]*1e-5 ; %rad/s - This is in body frame
...
Lv = [ 1 , 2 , 5 , 7 , 10 , 15 , 20 ] ;
zoffv = [ .2142 , .2807 , .5400 , .7628 , 1.1719 , 2.0533 , 3.1842 ] ;
COMv = [ zeros( 2 , length( zoffv ) ) ; -zoffv ] ;
Izv = [ .1271 , .1259 , .1221 , .1197 , .1159 , .1097 , .1034 ] ;
Ixv = [ 1.5571 , 5.1660 , 32.818 , 68.2014 , 152.0170 , 388.8188 , 766.7421 ] ;
Iyv = [ 1.5571 , 5.1660 , 32.818 , 68.2015 , 152.0172 , 388.8191 , 766.7426 ] ;


jj = 7 ; % change jj to calculate different lengths
Ix = Ixv( jj ) ;
Iy = Iyv( jj ) ;
Iz = Izv( jj ) ;
L = Lv( jj ) ;
...
tend = 92.75*60*10 ; 
% Set Torque , 0 is off, 1 is on, drag is slow with NRLMSISE00 turned on
% and magnetic is always slow
AtmoDrag = 1 ;
SRPDrag = 1 ;
GravGrad = 1 ;
Magnetic = 0 ;
torques = [ AtmoDrag ; SRPDrag ; GravGrad ; Magnetic ] ;

figure(6)
% comment out the plot to run with less memory but the individual torques
% are useful to see
hold on
% change final variable in the ode45 input from 0 to 1 to use NRLMSISE00
% model of atmosphere instead of table of nominal values for atmospheric
...
State(1:3,1) = zRot(-RAAN)*xRot(-inc)*zRot(-ArgOfPerigee)*State(1:3) ; %transforms the position into eci
State(4:6,1) = zRot(-RAAN)*xRot(-inc)*zRot(-ArgOfPerigee)*State(4:6) ; %transforms the velocity into eci
end
