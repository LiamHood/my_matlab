...
%Add specified base angles here          
base_angle = [0; 80; 120; 200; 240; 300] ;      %[degrees], Spacing of actuator pairs, base
base_angle = sort(base_angle);                  %sorted for calculation integrity

%Add specified top angles here          
...
motion_profile = 1;                                 % Choose motion profile
...
elseif configuration_6_3   
    ...
    for j = 1:length(theta)
        for i = 1:6
            ...
            b_rot(:,i,j)    = stewartrot(theta(j), phi(j), psi(j), b_i(:,i,j));
            L_vector(:,i,j) = P(:,j) + b_rot(:,i,j) - a_i(:,i,j) ;
            L_length(j,i)   = norm( L_vector(:,i,j) ) ;
        end
    end
end
