...
function problem3()
    ...
    fprintf('The maximum voltage required is %f which is less than the maximum \n', max(ein))
    fprintf('voltage of 28 V so the reaction wheels can complete a 30 degreee \n', ein_max)
    fprintf('slewing maneuver\n')
    figure
    subplot(2,1,1)
    plot(t, rad2deg(psi))
    xlabel('Time [s]')
